...
export class PlatformService {
  constructor(app = null) {
    ...
    // Register adapters during startup. Browser-backed adapters are initialized
    // only when a user starts work on that platform.
    this.adapterInitialization = this._initializeAdapters();
  ...
  async _initializeAdapters() {
    ...
    if (this.registry.registeredAdapters?.has('sparx')) {
      logger.info(
        { adapter: 'sparx' },
  ...
  normalizePlatform(platform) {
    ...
    const aliases = {
      sparxmaths: 'sparxMaths',
      sparxreader: 'sparxReader',
      sparxscience: 'sparxScience'
    };
  ...
  getPlatform(platform) {
    ...
    const definition =
      this.platformDefinitions.get(normalized) || {
  ...
  listPlatforms() {
    return Array.from(this.platformDefinitions.values());
  }
  ...
  async login(platform, payload = {}, context = {}) {
    ...
    if (!adapterId) {
      throw new Error('A platform is required.');
    }

    const loginPayload = {
      adapter: payload.adapter || adapterId,
    ...
    try {
      await this._ensureAdapterReady(adapterId);
  ...
  _getRegisteredAdapter(adapterId) {
    return (
      this.registry?.registeredAdapters?.get(adapterId)?.instance ||
      null
    );
  }

  async _ensureAdapterReady(adapterId) {
    const adapter = this._getRegisteredAdapter(adapterId);

    if (!adapter) {
      throw new Error(`No adapter is installed for '${adapterId}'.`);
    }
    ...
    if (!this.adapterReady.has(adapterId)) {
      this.adapterReady.set(adapterId, adapter.initialize());
    }

    try {
      await this.adapterReady.get(adapterId);
    } catch (error) {
      this.adapterReady.delete(adapterId);
      throw error;
    }
  }
  ...
  async shutdown() {
    ...
    const adapterIds = Array.from(
      this.registry?.registeredAdapters?.keys?.() || []
    );

    await Promise.allSettled(
      adapterIds.map(adapterId => this.registry.shutdown(adapterId))
    );
  }
