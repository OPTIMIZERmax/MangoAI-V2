import Sparx_Requesticator from './requesticatorBasic.js';
import { decode, encode } from './children/maths/sm_code.js';
import getTokenSparx from './login.js';
import getTokenRequest from './getTokenRequest.js';
import puppetQueue from '../../queues/puppeteerQueue.js';
        ...
        try {
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    // If the token request failed or returned Unauthorized, try puppeteer fallback
                    if (!newAuthToken || newAuthToken?.includes('Unauthorized')) {
                        if (newAuthToken?.includes('Unauthorized')) {
                            this.log.logToFile('Token request returned Unauthorized, attempting puppeteer fallback...');
                        } else {
                            this.log.logToFile('Token request returned no token, attempting puppeteer fallback...');
                        }
                    ...
                    if (newAuthToken && !newAuthToken.includes('Unauthorized')) {
                        ...
                        throw new Error("Authentication failed: No valid token returned");
                    }
